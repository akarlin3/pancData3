...
% Generate heatmaps and bar charts for core method comparison.
    ...
    fig1 = figure('Visible', 'off', 'Position', [100 100 950 800]);
    imagesc(results.mean_dice_matrix);
    cb1 = colorbar;
    ylabel(cb1, 'Dice Coefficient', 'FontSize', 10);
    colormap(parula);
    caxis([0 1]);
    ...
    set(gca, 'Position', [0.20 0.20 0.58 0.68]);
    xlabel('Core Method', 'FontSize', 11);
    ylabel('Core Method', 'FontSize', 11);
    title(sprintf('Mean Pairwise Dice Coefficient (%s)', dwi_type_name), 'FontSize', 13);
    ...
    print(fig1, fullfile(output_folder, sprintf('core_method_dice_heatmap_%s.png', dwi_type_name)), '-dpng', '-r150');
    close(fig1);

    % --- Figure 2: Mean HD95 Heatmap (if data available) ---
    % Use parula instead of hot for better text contrast (hot produces
    % yellow-on-yellow and dark-red-on-dark-red readability problems).
    if any(results.hd95_count(:) > 0)
        fig2 = figure('Visible', 'off', 'Position', [100 100 950 800]);
        imagesc(results.mean_hd95_matrix);
        cb2 = colorbar;
        ylabel(cb2, 'HD95 (mm)', 'FontSize', 10);
        colormap(parula);
        set(gca, 'XTick', 1:n_methods, 'XTickLabel', method_labels, ...
            'YTick', 1:n_methods, 'YTickLabel', method_labels, ...
            'XTickLabelRotation', 45, 'FontSize', 9, 'TickLabelInterpreter', 'none');
        ...
        xlabel('Core Method', 'FontSize', 11);
        ylabel('Core Method', 'FontSize', 11);
        title(sprintf('Mean Pairwise HD95 in mm (%s)', dwi_type_name), 'FontSize', 13);

        % Use white text on dark cells, black on light cells.
        % Parula maps low values to dark blue, high to yellow, so the
        % midpoint (~0.4 * range) is the crossover for text legibility.
        hd95_vals_flat = results.mean_hd95_matrix(~isnan(results.mean_hd95_matrix));
        hd95_max = max(hd95_vals_flat);
        hd95_min = min(hd95_vals_flat);
        if isempty(hd95_max), hd95_max = 1; hd95_min = 0; end
        hd95_mid = hd95_min + 0.4 * (hd95_max - hd95_min);
        for i = 1:n_methods
            for j = 1:n_methods
                val = results.mean_hd95_matrix(i, j);
                if ~isnan(val)
                    if val < hd95_mid
                        txt_color = [1 1 1];  % white text on dark (low-value) background
                    else
                        txt_color = [0 0 0];  % black text on light (high-value) background
                    end
        ...
        print(fig2, fullfile(output_folder, sprintf('core_method_hd95_heatmap_%s.png', dwi_type_name)), '-dpng', '-r150');
        close(fig2);
    end
