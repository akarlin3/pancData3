...
for di = 1:n_diff_metrics
    ...
    for dose_type = 1:2
        ...
        n_lc_pts = sum(lc_mask);
        n_lf_pts = sum(lf_mask);
        if isnan(r_lc)
            lc_corr_str = sprintf('LC (n=%d): n<3', n_lc_pts);
        else
            lc_corr_str = sprintf('LC (n=%d) r_s=%.2f %s', n_lc_pts, r_lc, format_p_value(p_lc));
        end
        if isnan(r_lf)
            lf_corr_str = sprintf('LF (n=%d): n<3', n_lf_pts);
        else
            lf_corr_str = sprintf('LF (n=%d) r_s=%.2f %s', n_lf_pts, r_lf, format_p_value(p_lf));
        end
        title(sprintf('%s vs Dose\n%s | %s', diff_names{di}, lc_corr_str, lf_corr_str), ...
            'FontSize', 10);

        % Warn when LF group is too small for reliable inference
        if n_lf_pts > 0 && n_lf_pts < 3
            yl = ylim; xl = xlim;
            text(xl(1) + 0.5*(xl(2)-xl(1)), yl(1) + 0.05*(yl(2)-yl(1)), ...
                sprintf('LF n=%d: unreliable for inference', n_lf_pts), ...
                'HorizontalAlignment', 'center', 'FontSize', 8, ...
                'Color', [0.7 0.1 0.1], 'FontWeight', 'bold', ...
                'BackgroundColor', [1 0.95 0.9], 'EdgeColor', [0.7 0.1 0.1], ...
                'Margin', 2);
        end
        if exist('OCTAVE_VERSION', 'builtin')
