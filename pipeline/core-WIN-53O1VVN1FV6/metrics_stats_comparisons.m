...
for s = 1:n_metric_sets
    ...
    for m = 1:num_rows
        ...
        for tp = 1:cols_to_plot
            ...
            % Count per-group sample sizes for annotation
            n_lc_tp = sum(g == 0);
            n_lf_tp = sum(g == 1);
            ...
            if ~isnan(p)
                ...
                if exist('OCTAVE_VERSION', 'builtin')
                    ...
                    boxplot(y, g, 'Labels', ...
                        {sprintf('LC (n=%d)', n_lc_tp), sprintf('LF (n=%d)', n_lf_tp)});
                end
                title_str = sprintf('%s - %s\np = %.3f (LC=%d, LF=%d)', ...
                    current_names{m}, time_labels{tp}, p, n_lc_tp, n_lf_tp);
                ...
                title(sprintf('%s - %s\nInsufficient Data (LC=%d, LF=%d)', ...
                    current_names{m}, time_labels{tp}, n_lc_tp, n_lf_tp), ...
                    'FontSize', 9, 'Color', [0.5 0.5 0.5]);
                % Show empty axes with message rather than hiding them
                set(gca, 'XTick', [], 'YTick', []);
                text(0.5, 0.5, sprintf('No data\n(LC=%d, LF=%d)', n_lc_tp, n_lf_tp), ...
