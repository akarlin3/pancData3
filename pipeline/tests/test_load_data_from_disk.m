classdef test_load_data_from_disk < matlab.unittest.TestCase
    ...
    %
    % Validates voxel-cache loading with type-specific files, un-typed
    % fallback for Standard only, and error on missing files.
    ...
    methods(Test)
        function test_loads_type_specific_file(testCase)
            ...
            cache_file = fullfile(testCase.TmpDir, 'pipeline_voxels_Standard.mat');
            save(cache_file, 'data_vectors_gtvp', 'data_vectors_gtvn');

            [gtvp, gtvn, sm] = load_data_from_disk( ...
                cache_file, '', '', 1, 'Standard');
            testCase.verifyEqual(gtvp.adc_vector, [1; 2; 3]);
            testCase.verifyEqual(gtvn.adc_vector, [4; 5]);
            testCase.verifyTrue(isempty(sm));
        end

        function test_fallback_for_standard(testCase)
            % Standard (dtype=1) should fall back to un-typed pipeline_voxels.mat.
            data_vectors_gtvp = struct('adc_vector', [10; 20]);
            data_vectors_gtvn = struct('adc_vector', [30]);
            fallback = fullfile(testCase.TmpDir, 'pipeline_voxels.mat');
            save(fallback, 'data_vectors_gtvp', 'data_vectors_gtvn');

            nonexistent = fullfile(testCase.TmpDir, 'pipeline_voxels_Standard.mat');
            [gtvp, ~, ~] = load_data_from_disk( ...
                nonexistent, fallback, '', 1, 'Standard');
            testCase.verifyEqual(gtvp.adc_vector, [10; 20]);
        end

        function test_no_fallback_for_dncnn(testCase)
            % dnCNN (dtype=2) must NOT fall back to un-typed file.
            data_vectors_gtvp = struct('adc_vector', [10; 20]);
            data_vectors_gtvn = struct('adc_vector', [30]);
            fallback = fullfile(testCase.TmpDir, 'pipeline_voxels.mat');
            save(fallback, 'data_vectors_gtvp', 'data_vectors_gtvn');

            nonexistent = fullfile(testCase.TmpDir, 'pipeline_voxels_dnCNN.mat');
