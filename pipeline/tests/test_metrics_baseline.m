classdef test_metrics_baseline < matlab.unittest.TestCase
    ...
    methods(Test)
        function testBaselineExecution(testCase)
            ...
            baseline = metrics_baseline(testCase.DataVectorsGTVp, testCase.DataVectorsGTVn, testCase.SummaryMetrics, testCase.ConfigStruct);

            % Check that valid_pts masks correctly (8 patients, all with finite lf)
            testCase.verifyEqual(length(baseline.valid_pts), 8);
            testCase.verifyTrue(all(baseline.valid_pts));

            % Check output matrix dimensions
            testCase.verifyEqual(size(baseline.ADC_abs, 1), 8);  % 8 patients
            testCase.verifyEqual(size(baseline.ADC_pct, 1), 8);
            testCase.verifyEqual(size(baseline.ADC_abs, 2), 3);  % 3 timepoints
            testCase.verifyEqual(size(baseline.ADC_pct, 2), 3);  % same shape as ADC_abs
        end
        ...
        function testOutlierCleaning(testCase)
            ...
            testCase.verifyTrue(isnan(baseline.ADC_abs(1, 1)));
            % Other elements should be intact
            testCase.verifyFalse(isnan(baseline.ADC_abs(2, 1)));
        end
