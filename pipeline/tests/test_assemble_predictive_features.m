...
%   - Index mapping after removal
    ...
    methods (Test)
        ...
        function test_24_columns_with_kurtosis_skewness(testCase)
            % When adc_kurt and adc_skew are provided, should have 24 features
            [ADC_abs, D_abs, f_abs, Dstar_abs, ADC_pct, D_pct, f_delta, Dstar_pct, ...
             d95g, v50g, d95a, v50a, d95d, v50d, d95f, v50f, d95ds, v50ds] = ...
                testCase.makeData();
            valid = true(testCase.nPat, 1);
            adc_kurt = randn(testCase.nPat, testCase.nTp);
            adc_skew = randn(testCase.nPat, testCase.nTp);
            ...
            testCase.verifyEqual(numel(full_names), 24, ...
                'Full feature name list should have 24 entries with kurtosis/skewness.');
            testCase.verifyTrue(any(strcmp(full_names, 'ADC_Kurt')), ...
                'Should contain ADC_Kurt.');
            testCase.verifyTrue(any(strcmp(full_names, 'ADC_Skew')), ...
                'Should contain ADC_Skew.');
        end

        function test_24_columns_post_treatment(testCase)
            % Post-treatment with kurtosis/skewness: dose excluded but kurt/skew kept
            [ADC_abs, D_abs, f_abs, Dstar_abs, ADC_pct, D_pct, f_delta, Dstar_pct, ...
             d95g, v50g, d95a, v50a, d95d, v50d, d95f, v50f, d95ds, v50ds] = ...
                testCase.makeData();
            valid = true(testCase.nPat, 1);
            ...
            [~, names, ~, full_names] = assemble_predictive_features( ...
                valid, testCase.nTp, testCase.nTp, 'Post', tempdir, ...
                ADC_abs, D_abs, f_abs, Dstar_abs, ADC_pct, D_pct, f_delta, Dstar_pct, ...
                d95g, v50g, d95a, v50a, d95d, v50d, d95f, v50f, d95ds, v50ds, ...
                adc_kurt, adc_skew);

            % Should have 14 features: 12 imaging + 2 kurtosis/skewness (no dose)
            testCase.verifyEqual(numel(full_names), 14, ...
                'Post-treatment with kurt/skew should have 14 features.');
            testCase.verifyTrue(any(strcmp(full_names, 'ADC_Kurt')));
            testCase.verifyTrue(any(strcmp(full_names, 'ADC_Skew')));
        end

        function test_backward_compat_without_kurtosis(testCase)
            % Without kurtosis/skewness args, should still produce 22 columns
            [ADC_abs, D_abs, f_abs, Dstar_abs, ADC_pct, D_pct, f_delta, Dstar_pct, ...
             d95g, v50g, d95a, v50a, d95d, v50d, d95f, v50f, d95ds, v50ds] = ...
                testCase.makeData();
            valid = true(testCase.nPat, 1);

            [~, ~, ~, full_names] = assemble_predictive_features( ...
                valid, 3, testCase.nTp, 'Fx3', tempdir, ...
                ADC_abs, D_abs, f_abs, Dstar_abs, ADC_pct, D_pct, f_delta, Dstar_pct, ...
                d95g, v50g, d95a, v50a, d95d, v50d, d95f, v50f, d95ds, v50ds);

            testCase.verifyEqual(numel(full_names), 22, ...
                'Without kurtosis/skewness, should have 22 features.');
        end
