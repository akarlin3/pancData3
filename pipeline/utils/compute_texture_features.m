function features = compute_texture_features(param_map, mask, n_levels, voxel_spacing, texture_3d)
...
%
%   GLRLM directional averaging:
%     For 3D inputs with more than one slice (and texture_3d enabled), the
%     GLRLM is computed in 13 unique 3D directions — the full set of
%     axis-aligned (3), face-diagonal (6), and body-diagonal (4) directions
%     — and features are averaged across all valid directions.  This
%     captures inter-slice texture information that 2D-only analysis misses.
%     For 2D inputs or single-slice 3D inputs (or when texture_3d is false),
%     the GLRLM falls back to 4 in-plane directions (0/45/90/135 degrees).
%
% Inputs:
...
%   texture_3d     - Logical flag to enable 3D GLRLM (13 directions).
%                    When false, always uses 2D GLRLM (4 directions).
%                    (default: true)
%
% Outputs:
...
%                           shape_elongation, shape_compactness
    ...
    if nargin < 5 || isempty(texture_3d), texture_3d = true; end
    ...
    if length(valid_pixels) >= 4
        ...
        if min_val == max_val
            ...
            % --- GLRLM features ---
            if use_3d_glrlm
                % 3D GLRLM: quantize full volume and compute in 13 directions
                all_vals_3d = double(param_map(mask > 0));
                all_vals_3d = all_vals_3d(isfinite(all_vals_3d));
                min_val_3d = min(all_vals_3d);
                max_val_3d = max(all_vals_3d);
                if min_val_3d == max_val_3d
                    features.glrlm_sre = NaN;
                    features.glrlm_lre = NaN;
                    features.glrlm_gln = NaN;
                    features.glrlm_rln = NaN;
                    ...
                    [features.glrlm_sre, features.glrlm_lre, features.glrlm_gln, ...
                        features.glrlm_rln, features.glrlm_rp] = compute_glrlm_3d(quantized_3d, mask, n_levels);
                end
            else
                % 2D GLRLM: 4 in-plane directions on best slice
                [features.glrlm_sre, features.glrlm_lre, features.glrlm_gln, ...
