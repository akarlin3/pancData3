...
%
    ...
    if strcmpi(plot_type, 'histogram')
        ...
        % Use a prominent background box so the message is not missed.
        if isempty(vals_lf) && ~isempty(vals_lc)
            yl = ylim;
            text(mean(xlim), yl(1) + 0.5*(yl(2) - yl(1)), ...
                'No LF events (n=0) — comparison not possible', ...
        ...
        elseif isempty(vals_lc) && ~isempty(vals_lf)
            yl = ylim;
            text(mean(xlim), yl(1) + 0.5*(yl(2) - yl(1)), ...
                'No LC events (n=0) — comparison not possible', ...
                'HorizontalAlignment', 'center', 'FontSize', 10, ...
                'Color', [0.7 0.1 0.1], 'FontWeight', 'bold', ...
                'BackgroundColor', [1 0.95 0.9], 'EdgeColor', [0.7 0.1 0.1], ...
                'Margin', 4);
    ...
    elseif strcmpi(plot_type, 'boxplot')
        ...
        % Annotate when only one outcome group is present in boxplot
        if numel(unique(lf_clean)) == 1
            yl = ylim;
            if all(lf_clean == 0)
                missing_str = 'No LF events (n=0) — single group only';
            else
                missing_str = 'No LC events (n=0) — single group only';
            end
            text(mean(xlim), yl(1) + 0.9*(yl(2) - yl(1)), ...
                missing_str, ...
