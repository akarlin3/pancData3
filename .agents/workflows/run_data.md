...
3. Use the `mcp_matlab-mcp_evaluate_matlab_code` tool to run each step of the pipeline individually:
   ...
   - `code`: `run_dwi_pipeline('config.json', {'metrics_stats'})`
   - `code`: `run_dwi_pipeline('config.json', {'metrics_survival'})`
4. Update `config.json` to set `"dwi_type": "dnCNN"`.
5. Use the `mcp_matlab-mcp_evaluate_matlab_code` tool to run each step of the pipeline individually again (load, sanity, visualize, metrics_baseline, metrics_longitudinal, metrics_dosimetry, metrics_stats, metrics_survival).
6. Update `config.json` to set `"dwi_type": "IVIMnet"`.
7. Use the `mcp_matlab-mcp_evaluate_matlab_code` tool to run each step of the pipeline individually again (load, sanity, visualize, metrics_baseline, metrics_longitudinal, metrics_dosimetry, metrics_stats, metrics_survival).
