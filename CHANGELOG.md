...
- **`pipeline/utils/optimize_adc_threshold.m`**: New MATLAB module — sweeps ADC threshold 0.8e-3 to 2.0e-3 in 13 steps and reports the threshold that maximises median Fx1 repeat Dice. Gated on `run_optimize_threshold` config flag. **Tactic 2 (Volume Inflection)** added: knee of the median sub-volume-fraction curve, found via discrete second derivative on a 3-point smoothed copy. Marks the biological transition between dense tumour core and surrounding tissue. **Tactic 3 (Outcome Significance)** added: per-threshold Wilcoxon rank-sum p-value of sub-volume fraction between LC (LF==0) and LF (LF==1) groups; reports the threshold with the smallest valid p-value (requires at least 3 patients per group). All three tactics are saved to the same `adc_threshold_optimization_<dwi>.mat` and overlaid as dashed reference lines on the optimisation PNG.
...
- **`analysis/report/sections/threshold_optimization.py`**: Renders the ADC threshold sweep as **three** independent H2 sections, one per optimisation tactic — Reproducibility (Dice, green highlight), Volume Inflection (knee, purple), and Outcome Significance (Wilcoxon p, orange) — so the analyst can compare which threshold is most stable, biologically meaningful, or outcome-discriminating. Each tactic skips itself silently when its data is absent, so older `.mat` files without the new fields still render the original Dice section.
