...
│   └── tests/                          # Python test suite — 27 test files, 760 tests (pytest)
├── .agents/
...
**Python Test Suite (pytest):** 27 test files with 760 tests in `analysis/tests/`. Run with `cd analysis/tests && python -m pytest -v`.
...
| `test_generate_report.py` | Stub pointing to the split test files below |
| `test_generate_report_formatting.py` | Series normalisation, sig tags, section headers, forest plot, effect size, copy helpers, figure/table numbering |
| `test_generate_report_sections.py` | Section builders: data completeness, feature overlap, power analysis, patient flow, sensitivity analysis, feature stability, appendix |
| `test_generate_report_manuscript.py` | Manuscript findings, reporting checklist, results draft, journal guide, effect sizes in findings |
| `test_generate_report_figures.py` | Figure gallery: PNG embedding, DWI type grouping |
| `test_generate_report_integration.py` | Full HTML report generation, BibTeX export, new section integration, data quality, Cox PH direction, correlations |
