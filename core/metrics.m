...
adc_wCV = squeeze(nanstd(adc_mean_rpt,[],2))./squeeze(mean(adc_mean_rpt,2, 'omitnan'));
adc_wCV(n_rpt<2) = nan;

% --- ADC repeatability (low-ADC subregion mean) ---
adc_wCV_sub = squeeze(nanstd(adc_sub_rpt,[],2))./squeeze(mean(adc_sub_rpt,2, 'omitnan'));
adc_wCV_sub(n_rpt<2) = nan;

% Print ADC wCV: whole-GTV and subregion, for Standard and dnCNN pipelines
fprintf('ADC   wCV (whole gtv) = %2.2f%s%2.2f%%, dnCNN: %2.2f%s%2.2f%%\n',mean(adc_wCV(:,1), 'omitnan')*100,char(177),nanstd(adc_wCV(:,1))*100,mean(adc_wCV(:,2), 'omitnan')*100,char(177),nanstd(adc_wCV(:,2))*100)
fprintf('ADC   wCV (subregion) = %2.2f%s%2.2f%%, dnCNN: %2.2f%s%2.2f%%\n',mean(adc_wCV_sub(:,1), 'omitnan')*100,char(177),nanstd(adc_wCV_sub(:,1))*100,mean(adc_wCV_sub(:,2), 'omitnan')*100,char(177),nanstd(adc_wCV_sub(:,2))*100)

% --- IVIM parameter repeatability (D, f, D*) ---
% Each has 3 DWI types: Standard, dnCNN, IVIMnet
d_wCV = squeeze(nanstd(d_mean_rpt,[],2))./squeeze(mean(d_mean_rpt,2, 'omitnan'));
d_wCV(repmat(n_rpt,[1,3])<2) = nan;  % Mask patients with <2 repeats

f_wCV = squeeze(nanstd(f_mean_rpt,[],2))./squeeze(mean(f_mean_rpt,2, 'omitnan'));
f_wCV(repmat(n_rpt,[1,3])<2) = nan;

dstar_wCV = squeeze(nanstd(dstar_mean_rpt,[],2))./squeeze(mean(dstar_mean_rpt,2, 'omitnan'));
dstar_wCV(repmat(n_rpt,[1,3])<2) = nan;

% Print IVIM wCV for all three processing pipelines (char(177) = ± symbol)
fprintf('D     wCV = %2.2f%s%2.2f%%, dnCNN: %2.2f%s%2.2f%%, IVIMnet: %2.2f%s%2.2f%%\n',mean(d_wCV(:,1), 'omitnan')*100,char(177),nanstd(d_wCV(:,1))*100,mean(d_wCV(:,2), 'omitnan')*100,char(177),nanstd(d_wCV(:,2))*100,mean(d_wCV(:,3), 'omitnan')*100,char(177),nanstd(d_wCV(:,3))*100)
fprintf('f     wCV = %2.2f%s%2.2f%%, dnCNN: %2.2f%s%2.2f%%, IVIMnet: %2.2f%s%2.2f%%\n',mean(f_wCV(:,1), 'omitnan')*100,char(177),nanstd(f_wCV(:,1))*100,mean(f_wCV(:,2), 'omitnan')*100,char(177),nanstd(f_wCV(:,2))*100,mean(f_wCV(:,3), 'omitnan')*100,char(177),nanstd(f_wCV(:,3))*100)
fprintf('Dstar wCV = %2.2f%s%2.2f%%, dnCNN: %2.2f%s%2.2f%%, IVIMnet: %2.2f%s%2.2f%%\n',mean(dstar_wCV(:,1), 'omitnan')*100,char(177),nanstd(dstar_wCV(:,1))*100,mean(dstar_wCV(:,2), 'omitnan')*100,char(177),nanstd(dstar_wCV(:,2))*100,mean(dstar_wCV(:,3), 'omitnan')*100,char(177),nanstd(dstar_wCV(:,3))*100)
    ...
    for i = 1:n_eval
        ...
        for j = i+1:n_eval
            ...
            if ti < tj
                ...
                elseif ei == 2 && ej == 1
                    % Competing-risk pair: j has event of interest after i's
                    % competing event. j should have higher progression risk.
                    comparable = true;
            ...
            elseif tj < ti
                if ej == 1
                    ...
                    better_survival_idx = ii;
                elseif ej == 2 && ei == 1
                    % Competing-risk pair: i has event of interest after j's
                    % competing event. i should have higher progression risk.
                    comparable = true;
                    worse_survival_idx = ii;
                    better_survival_idx = jj;
                end
...
for p_idx = 1:n_vp
    ...
    try
        [B_en, FI_en] = lassoglm(X_train, pat_event_train, 'binomial', ...
            'Alpha', 0.5, 'CV', cv_fold_ids, 'Standardize', true, ...
            'Options', statset('UseParallel', false));
        
        idx_opt = FI_en.IndexMinDeviance;
        ...
        % If lassoglm fails to converge, fall back to using all collinear-filtered features
    end
    % --- END Elastic Net ---
    ...
    % Uses leave-one-out training times so there is no data leakage.
    ...
    if surv_event_all(i) == 1 && ~isempty(oof_risk_history{i})
        ...
        for j = 1:n_vp
            if i ~= j && surv_event_all(j) == 2 && surv_time_all(j) < T_i && ~isempty(oof_risk_history{j})
                T_j = surv_time_all(j);
                risk_j = NaN;
                idx_j = find(T_j > oof_risk_history{j}.t_start & T_j <= oof_risk_history{j}.t_stop, 1, 'last');
...
fprintf('  Absolute Size: %.1f ± %.1f cc\n', mean(sub_vol_cc, 'omitnan'), nanstd(sub_vol_cc));
